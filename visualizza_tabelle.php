...
<input class=\"btn btn-primary\" type=\"submit\" name=\"cont_op\" value=\"".mex("Continua",$pag)."\">
...
<input type=\"hidden\" name=\"opz_cerc_pren\" value=\"$opz_cerc_pren\">
<input type=\"hidden\" name=\"id_utente_vedi\" value=\"$id_utente_vedi\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"torna_alla_tab\" value=\"".mex("Annulla",$pag)."\">
</div></form>";
...
if ($sel_tab_prenota == "correnti" or $sel_tab_prenota == "future") {
$oggi = date("Y-m-d",(time() + (C_DIFF_ORE * 3600)));
$oggi = esegui_query("select datainizio,datafine from $tableperiodi where datainizio <= '$oggi' and datafine > '$oggi' ");
...
echo "<div class=\"widgetbox\">
<h4 class=\"widgettitle\">".mex("Tutte le prenotazioni",$pag)."</h4>
<div class=\"widgetcontent\">
...
<input type=\"hidden\" name=\"id_utente_vedi\" value=\"$id_utente_vedi\">
<span class=\"field\">
<select class=\"uniformselect\" name=\"opz_cerc_pren\">
...
echo " <input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Vedi",$pag)."\">
</div></form></td>";
...
<input type=\"hidden\" name=\"id_utente_vedi\" value=\"$id_utente_vedi\">
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Vedi tutte le prenotazioni",$pag)."\">
</div></form></td>";
...
".mex("Profilo",$pag)."
<span class=\"field\">
<select class=\"uniformselect\" name=\"id_utente_vedi\">
<option value=\"$id_utente_vedi\" selected>$nome_utente_vedi</option>
$option_select
</select></span>
<input class=\"btn btn-primary\" type=\"submit\" name=\"cambia_ut_vedi\" value=\"".mex("Vedi",$pag)."\">
</div></form></td>";
...
<input class=\"btn btn-primary\" type=\"submit\" name=\"cambi_colori\" value=\"".mex("Senza colori",$pag)."\">";
} # fine if (!$senza_colori)
else echo "<input class=\"btn btn-primary\" type=\"submit\" name=\"cambi_colori\" value=\"".mex("Con colori",$pag)."\">";
...
esegui_query("insert into $tabletransazioni (idtransazioni,idsessione,tipo_transazione,anno,dati_transazione1,dati_transazione2,dati_transazione3,dati_transazione4,dati_transazione5,dati_transazione6,dati_transazione7,dati_transazione8,dati_transazione9,dati_transazione10,dati_transazione11,dati_transazione12,ultimo_accesso) 
values ('$id_transazione','$id_sessione','tab_p','$anno','".aggslashdb($pag_pren_corr)."','".aggslashdb($pag_pren_succ)."','".aggslashdb($pag_pren_prec)."','".aggslashdb($ordine_prenota)."','".aggslashdb($mos_tut_dat)."','".aggslashdb($cerca_prenota)."','".aggslashdb($opz_cerc_pren)."','".aggslashdb($cerca_ini)."','".aggslashdb($cerca_fine)."','".aggslashdb($id_utente_vedi)."','".aggslashdb($senza_colori)."','".aggslashdb($lista_prenota)."','$ultimo_accesso')");
...
if (substr($lista_date1[1],0,10) == $cerca_fine) {
$lista_date0 = explode("<option value=\"",$lista_date0[0]);
$num_lista_date0 = count($lista_date0);
if ($num_lista_date0 > 1) {
$n_cerca_ini = substr($lista_date0[($num_lista_date0 - 1)],0,10);
$freccia_sx = "<a href=\"$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;tipo_tabella=$tipo_tabella&amp;senza_colori=$senza_colori&amp;mos_tut_dat=$mos_tut_dat&amp;ordine_prenota=$ordine_prenota&amp;id_utente_vedi=$id_utente_vedi&amp;cerca_prenota=SI&amp;cerca_ini=$n_cerca_ini&amp;cerca_fine=$cerca_ini&amp;opz_cerc_pren=$opz_cerc_pren&amp;lista_prenota=$lista_prenota\">&lt;--</a>&nbsp;";
} # fine if ($num_lista_date0 > 1)
if (count($lista_date1) > 2) {
$n_cerca_fine = substr($lista_date1[2],0,10);
$freccia_dx = "&nbsp;<a href=\"$pag?anno=$anno&amp;id_sessione=$id_sessione&amp;tipo_tabella=$tipo_tabella&amp;senza_colori=$senza_colori&amp;mos_tut_dat=$mos_tut_dat&amp;ordine_prenota=$ordine_prenota&amp;id_utente_vedi=$id_utente_vedi&amp;cerca_prenota=SI&amp;cerca_ini=$cerca_fine&amp;cerca_fine=$n_cerca_fine&amp;opz_cerc_pren=$opz_cerc_pren&amp;lista_prenota=$lista_prenota\">--&gt;</a>";
} # fine if (count($lista_date1) > 2)
} # fine if (substr($lista_date1[0],0,10) == $cerca_fine)
...
echo "<h4 class=\"widgettitle\">Data Table</h4>
<table class=\"table table-bordered responsive\" id=\"tpren$num_tab\"  >
<thead><tr><th";
...
if ($colonna_cognome) echo "<th class=\"head0\">".str_replace("_","&nbsp;",mex("Cognome_cliente",$pag))."</th>";
if ($colonna_inizio) {
echo "<th>";
...
if ($colonna_persone) echo "<th style=\"font-size: x-small;\">".mex("Pers.",$pag)."</th>";
...
echo "".ucfirst(mex("porre per le prenotazioni selezionate",$pag))."<span class=\"field\"> <select class=\"uniformselect\" name=\"somma_pagata\">";
...
echo "</select></span>
 <input class=\"btn btn-primary \" type=\"submit\" name=\"cambia_pagato\" value=\"".ucfirst(mex("cambia",$pag))."\">
<small><small><br><br></small></small>";
} # fine if ($priv_mod_pagato != "n")
echo "<input class=\"btn btn-primary\" type=\"submit\" name=\"subtotale_selezionate\" value=\"".mex("Calcola il subtotale delle prenotazioni selezionate",$pag)."\">
</div>";
} # fine if ($show_bar != "NO")
...
<input id=\"smt_prenota_mod\" class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Modifica come gruppo le prenotazioni mostrate in tabella",$pag)."\">
</div></form></div>";
} # fine if ($show_bar != "NO")
...
<span class=\"field\"><select class=\"uniformselect\" name=\"numero_contratto\">$option_num_contr</select></span>

<input class=\"btn btn-primary\" type=\"submit\" name=\"vedi_contr\" value=\"".ucfirst(mex("visualizza",$pag))."\">
</div></form></div>";
...
<input class=\"btn btn-primary\" type=\"submit\" name=\"cancella\" value=\"".mex("SI",$pag)."\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"torna_alla_tab\" value=\"".mex("NO",$pag)."\">
</div></form>";
} # fine if (!$cancella)
...
echo "<div class=\"widgetbox\"><h4 class=\"widgettitle\">".mex("Tutte le entrate del",$pag)." $anno</h4>
<div class=\"widgetcontent\">
<a name=\"entr_cassa$id_cassa\"></a><br>";
...
<thead>
<tr><th>".str_replace("_","&nbsp;",mex("Natura_entrata",$pag))."</th>
<th>".str_replace("_","&nbsp;",mex("Importo",$pag))."</th>
<th>".str_replace("_","&nbsp;",mex("Inserita_da",$pag))."</th>
<th><small>".str_replace(" ","&nbsp;",mex("Metodo pagamento",$pag))."</small></th>
<th>".str_replace("_","&nbsp;",mex("Data_di_inserimento",$pag))."</th>";
if ($priv_ins_entrate == "s") echo "<th>".mex("Cancella",$pag)."</th>";
echo "</tr></thead><tbody>";
...
echo "</tr></tbody></table><div style=\"text-align: center;\">* ".mex("entrata presa dalle prenotazioni",$pag)."$stringa_pagine
<br></div></div>";
echo "<div class=\"widgetbox\">
<h4 class=\"widgettitle\">".mex("Tutte le spese del",$pag)." $anno</h4>
<div class=\"widgetcontent\">
<a name=\"usci_cassa$id_cassa\"></a><br>";
if ($id_cassa == 1) echo mex("nella cassa principale",$pag);
else echo mex("nella cassa chiamata",$pag)." <em>$nome_cassa</em>";
echo ".</h3>";
...
<table class=\"table table-bordered responsive\">
<thead>
<tr><th>".str_replace("_","&nbsp;",mex("Natura_spesa",$pag))."</th>
<th>".str_replace("_","&nbsp;",mex("Importo",$pag))."</th>
<th>".str_replace("_","&nbsp;",mex("Inserita_da",$pag))."</th>
<th><small>".str_replace(" ","&nbsp;",mex("Metodo pagamento",$pag))."</small></th>
<th>".mex("Data_di_inserimento",$pag)."</th>";
if ($priv_ins_spese == "s") echo "<th>".mex("Cancella",$pag)."</th>";
echo "</tr></thead>";
if ($stringa_pagine and $pagina_prenota > 1) echo $stringa_puntini_tab;
...
echo "</tr></table>$stringa_pagine<br><br>";
...
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Crea",$pag)."\">
</div></form>";
if ($opt_casse_canc) {
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"$pag\"><div class=\"txtcenter\" style=\"padding: 2px 0 2px 0;\">
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
...
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"indietro\" value=\"".mex("Inserisci nuove spese o entrate",$pag)."\">
</div></form></div></td>";
...
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"indietro\" value=\"".mex("Storia entrate-uscite prenotazioni",$pag)."\">
...
<input type=\"hidden\" name=\"tipo_tabella\" value=\"$tipo_tabella\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"continua\" value=\"".mex("Continua",$pag)."\">
</div></form><br>
<form accept-charset=\"utf-8\" method=\"post\" action=\"visualizza_tabelle.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"$tipo_tabella\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"torna\" value=\"".mex("Torna indietro",$pag)."\">
...
echo "<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Torna indietro",$pag)."\">
</div></form>";
...
echo "<div class=\"widgetbox\">
<h4 class=\"widgettitle\">".mex("Tabella con periodi e relative tariffe del",$pag)." $anno.</h4>
<div class=\"widgetcontent\">";
if ($form_tabella) {
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"visualizza_tabelle.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
...
echo "<br><table class=\"table table-bordered responsive\">";
$riga_tariffe_tab = "<thead><tr><th><a href=\"./visualizza_tabelle.php?anno=$anno&amp;id_sessione=$id_sessione&amp;tipo_tabella=periodi&amp;mos_tutti_per=SI&amp;form_tabella=$form_tabella&amp;$lista_tariffe_sel\">N°</a></th>
<th>".str_replace("_","&nbsp;",mex("Inizio_$parola_settimana",$pag))."</th>
<th>".str_replace("_","&nbsp;",mex("Fine_$parola_settimana",$pag))."</t>";
...
$riga_tariffe_tab .= "</tr><tbody>";
echo $riga_tariffe_tab;

if (!$form_tabella and !$tariffe_sel) {
echo "<tr><td colspan=\"3\">
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Mostra solo tariffe selezionate",$pag)."\">
</td>";
for ($numtariffa = 1 ; $numtariffa <= $dati_tariffe['num'] ; $numtariffa++) {
if ($priv_vedi_tab_periodi != "p" or $attiva_tariffe_consentite == "n" or $tariffe_consentite_vett[$numtariffa] == "SI") {
...
echo "<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Modifica i campi della tabella",$pag)."\">
</div></form><br>";
if ($id_utente == 1) {
$data_fine_periodi = esegui_query("select max(idperiodi) from $tableperiodi");
$data_fine_periodi = risul_query($data_fine_periodi,0,0);
...
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"periodi\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"aggiungi\" value=\"".mex("Aggiungi periodi",$pag)."\">
...
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"periodi\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"cancella_anno\" value=\"".mex("Cancella",$pag)."\">
...
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Modifica i prezzi",$pag)."\">
</div></div></form><br>";
} # fine else if (!$form_tabella)
} # fine if ($priv_mod_tariffe != "n")

echo "<br><a name=\"tab_costi_agg\"></a><br><h4 class=\"widgettitle\">".mex("Tabella con i costi aggiuntivi del",$pag)." $anno.</h3>
<br><table class=\"table table-bordered responsive\" >
<thead><tr><th>".mex("Nome",$pag)."</th>
<th>".mex("Tipo",$pag)."</th>
<th>".mex("Prezzo",$pag)."</th>
<th>".mex("Regole",$pag)."</th>
<th style=\"font-size: small;\">".mex("Tariffe abbinate",$pag)."</th>
<th style=\"font-size: x-small;\">".mex("Incompatibilità",$pag)."</th>";
...
if ($beni_inv_esist) echo "<th style=\"font-size: x-small;\">".mex("Beni inventario",$pag)."</th>";
echo "<th style=\"font-size: small;\">".mex("Periodi_permessi",$pag)."</th>";
if ($priv_mod_costo_agg != "n" or $priv_canc_costi_agg != "n") echo "<th>".mex("Modifica",$pag)."</th>";
echo "</tr></thead><tbody>";
...
echo "<a name=\"tab_caparre\"><br></a><h4 class=\"widget title\">".mex("Caparre, commissioni e tasse del",$pag)." $anno.</h4>
<br><table class=\"table table-bordered responsive\">";
echo "<thead><tr><th rowspan=\"2\">".mex("Tariffa",$pag)."</th>
<th colspan=\"2\">&nbsp;".mex("Caparra",$pag)."&nbsp;</th>
<th colspan=\"2\">&nbsp;".mex("Commissioni",$pag)."&nbsp;</th>
<th rowspan=\"2\">&nbsp;".mex("Tasse",$pag)."&nbsp;</th></tr>
<tr><th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".mex("Valore",$pag)."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</hd>
<th>".str_replace("_","&nbsp;",mex("Arrotondato_a",$pag))."</th>
<th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".mex("Valore",$pag)."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
<th>".str_replace("_","&nbsp;",mex("Arrotondato_a",$pag));
...
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"insprezzi\" value=\"".mex("Vai alla pagina di inserzione prezzi",$pag)."\">
</div></form></div><br>";
...
$tableprenota_mostra = $PHPR_TAB_PRE."prenota".$anno_mostra;
if ($anno_mostra == $anno_corr) $prenota = esegui_query("select distinct idclienti from $tableprenota_mostra where iddatafine > '$idperiodocorrente' ");
else  $prenota = esegui_query("select distinct idclienti from $tableprenota_mostra");
$num_prenota = numlin_query($prenota);
for ($num2 = 0 ; $num2 < $num_prenota ; $num2++) $cliente_attivo[risul_query($prenota,$num2,'idclienti')] = 1;
...
echo "
".mex("Si è sicuri di voler <div style=\"display: inline; color: red;\"><b>cancellare",$pag)." $num_cc_canc ".mex("carte di credito</b></div> associate a clienti che non hanno nessuna prenotazione nel futuro",$pag)."?<br><br>";
if ($num_cc_canc) {
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"visualizza_tabelle.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
...
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Continua",$pag)."\">
</div></form>";
} # fine if ($num_cc_canc)
echo "<form accept-charset=\"utf-8\" method=\"post\" action=\"visualizza_tabelle.php\"><div>
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"$tipo_tabella\">
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Torna indietro",$pag)."\">
</div></form><br><br>";
} # fine if (!$continua)
else {
...
<input type=\"hidden\" name=\"tipo_tabella\" value=\"$tipo_tabella\">
<input type=\"hidden\" name=\"cancella_cc_passate\" value=\"SI\">
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Cancella le carte di credito di prenotazioni passate",$pag)."\">
</div></form></td>";
} # fine if (numlin_query($cert_cc))
} # fine if (function_exists('openssl_pkey_new'))
...
<input type=\"hidden\" name=\"tipo_tabella\" value=\"$tipo_tabella\">
<input type=\"hidden\" name=\"cancella_clienti\" value=\"SI\">
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Cancella i clienti senza prenotazioni",$pag)."\">
</div></form></td>";
...
if ($cognome_cerca) {
$cognome_cerca = aggslashdb($cognome_cerca);
...
<thead><tr><th>N°</th>
<th>".mex("Cognome",$pag)."</th>
<th>".mex("Nome",$pag)."</th>
<th>".mex("Nazione",$pag)."</th>
<th>".mex("Città",$pag)."</th>";
if ($priv_vedi_telefoni == "s") {
echo "<th>".mex("Telefono",$pag)."</th>
<th>".mex("Email",$pag)."</th>";
} # fine if ($priv_vedi_indirizzo == "s")
if ($priv_vedi_indirizzo == "s") {
echo "<th>".mex("CAP",$pag)."</th>
<th>".mex("Indirizzo",$pag)."</th>";
} # fine if ($priv_vedi_indirizzo == "s")
/*echo "<td>".mex("Fax",$pag)."</td>
<td class=\"smallfont\">".mex("Data_di_nascita",$pag)."</td>
<td>".mex("2°telefono",$pag)."</td>";*/
echo "<th>".mex("Commento",$pag)."</th>";
if ($cognome_cerca) echo "<th class=\"xsmallfont\">".mex("Prenotazioni anno corrente",$pag)."</th>";
echo "</tr></thead><tbody>";
...
echo "<div class=\"widgetbox\">
<h4 class=\"widgettitle\">".mex("Tabella con le regole di assegnazione 1 del",$pag)." $anno.</h4>
<div class=\"widgetcontent\">
<br><table class=\"table table-bordered responsive\">
<thead>
<tr><th><small>".mex("Appartamento",'unit.php')." / ".ucfirst(mex("tariffa",$pag))."</small></th>
<th>".str_replace("_","&nbsp;",mex("Inizio_$parola_settimana",$pag))."</th>
<th>".str_replace("_","&nbsp;",mex("Fine_$parola_settimana",$pag))."</th>
<th>".mex("Motivazione",$pag)."</th>";
if ($id_utente == 1 and $installazione_subordinata != "SI") echo "<th>".mex("Cancella",$pag)."</th>";
echo "</tr></thead><tbody>";
...
echo "<br><br>
<h4 class=\"widgettitle\">".mex("Tabella con le regole di assegnazione 2 del",$pag)." $anno.</h4>
<br><table class=\"table table-bordered responsive\" >
<thead><tr><th>".mex("Tariffa",$pag)."</th>
<th>".mex("Appartamenti",'unit.php')."</th>";
if ($id_utente == 1 and $installazione_subordinata != "SI") echo "<th>".mex("Cancella",$pag)."</th>";
echo "</tr></thead><tbody>";
...
echo "<br><br> <h4 class=\"widgettitle\">".mex("Tabella con le regole di assegnazione 3 del",$pag)." $anno.</h4>
<br><table class=\"table table-bordered responsive\"><thead>
<tr><th>".mex("Tariffa",$pag)."</th>
<th>".mex("Utente",$pag)."</th>";
if ($installazione_subordinata != "SI") echo "<th>".mex("Cancella",$pag)."</th>";
echo "</tr></thead><tbody>";
...
echo "<br><br><h4 class=\"widgettitle\">".mex("Tabella con le regole di assegnazione 4 del",$pag)." $anno.</h4>
<br><table class=\"table table-bordered responsive\" >
<thead><tr><th>".mex("Tariffa",$pag)."</th>
<th><small>".str_replace(" ","&nbsp;",mex("Numero di persone",$pag))."</small></th>";
if ($installazione_subordinata != "SI") echo "<th>".mex("Cancella",$pag)."</th>";
echo "</tr></thead><tbody>";
...
<input type=\"hidden\" name=\"anno\" value=\"$anno\">
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"vai\" value=\"".mex("Inserisci nuove regole",$pag)."\">
</div></form></div>";
...
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"appartamenti\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"continua\" value=\"".mex("Torna indietro",$pag)."\">
<hr style=\"width: 95%\">
</div></form>";
...
echo "<h4 class=\"widgettitle\">".mex("Tabella con tutti gli appartamenti",'unit.php').".</h4><br>";
...
echo "<table class=\"table table-bordered responsive\">
<thead>
<tr><th>".mex("Appartamento",'unit.php')."</th>
<th>".mex("Casa",$pag)."</th>
<th>".mex("Piano",$pag)."</th>
<th style=\"font-size: small;\">".str_replace("_","&nbsp;",mex("Capienza_massima",$pag))."</th>
<th>".mex("Priorità",$pag)."</th>";
if (!$form_tabella) {
if ($priv_vedi_inv_app != "n") echo "<th>".ucwords(mex("inventario",$pag))."</th>";
echo "<th>".mex("Commento",$pag)."</th>";
if ($id_utente == 1 and $installazione_subordinata != "SI") echo "<th>".mex("Modifica",$pag)."</th>";
} # fine if (!$form_tabella)
echo "</tr></thead><tbody>";
...
<input type=\"hidden\" name=\"tipo_tabella\" value=\"appartamenti\">
<input type=\"hidden\" name=\"form_tabella\" value=\"SI\">
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Modifica tutti i campi della tabella",$pag)."\">
</div></form><br>";
...
echo "<table class=\"table table-bordered responsive\"><thead>";
...
<input type=\"hidden\" name=\"tipo_tabella\" value=\"appartamenti\">
<input type=\"hidden\" name=\"mostra_griglia_app\" value=\"1\">
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Griglia appartamenti vicini",'unit.php')."\">
</div></form><br>";
...
echo "<div class=\"widgetbox\">
<h4 class=\"widgettitle\">".mex("Magazzini ed appartamenti",'unit.php')."</h4>
<div class=\"widgetcontent\">
                    <a name=\"tab_magazzini\"></a>
<br>
...
<tr><th>".mex("Nome",$pag)."</th>
<th>".mex("Tipo",$pag)."</th>
<th>".mex("Descrizione",$pag)."</th>";
if ($id_utente == 1 and $num_tutti_utenti > 1) echo "<th>".mex("Utente",$pag)."</th>";
echo "<th>".mex("Modifica",$pag)."</th></tr>";
...
echo "</td><td><input class=\"btn btn-primary\" type=\"submit\" name=\"aggiungi_mag\" value=\"".mex("aggiungi",$pag)."\"></td></tr>";
} # fine if ($priv_vedi_inv_mag != "n" and $priv_ins_mag != "n")

echo "</tbody></table></div></form></div></div><br><br>";
} # fine if (($priv_vedi_inv_mag != "n" or $priv_vedi_inv_app != "n") and...

if ($priv_vedi_beni_inv != "n") {

echo "<a name=\"tab_beni_inv\"></a>
$frase_tab_beni_inv
...
echo "<table class=\"table table-bordered responsive\">
<thead><tr><th>".mex("Nome",$pag)."</th>
<th>".mex("Codice",$pag)."</th>
<th>".mex("Descrizione",$pag)."</th>";
if (!$modifica_tab_beni_inv) echo "<th>".mex("Mancanti",$pag)."</th>";
if ($id_utente == 1 and $num_tutti_utenti > 1) echo "<th>".mex("Utente",$pag)."</th>";
if (!$modifica_tab_beni_inv) echo "<th>".mex("Modifica",$pag)."</th>";
echo "</tr></thead><tbody>";
...
echo "<tr><td><input type=\"text\" name=\"n_nome_bene\" size=\"18\"></td>
...
echo "<td>".mex("<input class=\"btn btn-primary\" type=\"submit\" name=\"aggiungi_bene\" value=\"".mex("aggiungi",$pag)."\">",$pag)."</td></tr>";
...
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Modifica i beni",$pag)."\"></div>";
echo "</div></form>";
...
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Modifica i beni già inseriti",$pag)."\">
</div></form>";
} # fine if ($priv_ins_beni_inv != "n" and !$modifica_tab_beni_inv)
...
echo "<div class=\"widgetbox\"><h4 class=\"widgettitle\">".ucfirst(mex("documenti salvati",$pag)).".</h4>  
<div class=\"widgetcontent\">";
...
echo "<br><h4 class=\"widgettitle\"> $nome_contr ".mex("in",$pag)." $dir_salva</h4>
$stringa_pagine
<table class=\"table table-bordered responsive\">
<thead>
<tr><th>".ucfirst(mex("numero",$pag))."</th>
<th>".ucfirst(mex("nome file",$pag))."</th>
<th>".ucfirst(mex("prenotazioni",$pag))."</th>
<th>".ucfirst(mex("azione",$pag))."</th></tr>";
if ($stringa_pagine and $pagina_prenota != 1) echo $stringa_puntini_tab;
...
if ($tab_contr) {
if (($num_pagine_prenota == 1 or $pagina_prenota == $num_pagine_prenota) and ($attiva_contratti_consentiti == "n" or $contratti_consentiti_vett[$num_contr] == "SI") and $ultimo_n_contr == $max_contr_esist) {
...
<input class=\"btn btn-primary\" type=\"submit\" value=\"".ucfirst(mex("elimina",$pag))."\">
</div></form></td></tr>";
...
<input class=\"btn btn-primary\" type=\"submit\" name=\"upload_backup\" value=\"".ucfirst(mex("fai l'upload",$pag))."\">
</div></form></td></tr>";
} # fine if ($attiva_contratti_consentiti == "n" or...
...
$istogramma = "<h4 class=\"widgettitle\">$titolo</h4>
<table class=\"table table-bordered responsive\">
...
<table class=\"tabel table-bordered responsive\".$colore_col[$num1].";"><tr><td></td></tr></table>
</td>
...
$costo_agg_tot = 0;
for ($numca = 0 ; $numca < $dati_cap['num'] ; $numca++) {
$costo_agg_parziale = (double) calcola_prezzo_totale_costo($dati_cap,$numca,$id_data_inizio,$id_data_fine,$dati_cap[$numca]['settimane'],$dati_cap[$numca]['moltiplica_costo'],$tariffa[1],$tariffe_settimanali_int,((double) $tariffa[1] + $costo_agg_tot - (double) $sconto),$caparra,$numpersone);
$costo_agg_tot = (double) $costo_agg_tot + $costo_agg_parziale;
if ($dati_cap[$numca]['tasseperc']) {
...
echo "<div class=\"widgetbox\"><h4 class=\"widgettitle\">".mex("Statistiche",$pag)."</h4><div class=\"widgetcontent\">";
...
if ($nome_tariffa_trovata) echo "<hr class=\"widgettitle\">(".mex("appartamenti della tariffa",'unit.php')." $nome_tariffa_trovata)</h4>";
...
echo " <input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Cambia",$pag)."\">
</div></form>";
...
</div></form></div><div style=\"height: 20px\"></div></div></div>";
