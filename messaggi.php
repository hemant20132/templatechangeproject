...
$max_mess = esegui_query("select max(id) from $tablemessaggi");
...
if ($mittente == $mittente_email_tm[$num2] or stristr($mittente,"<".$mittente_email_tm[$num2].">") or !$mittente_email_tm[$num2]) {
if (stristr($oggetto,$oggetto_email_tm[$num2])) {
if ($therad_email_tm[$num2] == "tutti" or (!$risposta_a and !$riferimento)) {
$inserisci_mess = 1;
$mess_scaricati++;
break;
} # fine if ($therad_email_tm[$num2] == "tutti" or...
} # fine if (stristr($oggetto,$oggetto_email_tm[$num2]))
} # fine if ($mittente == $mittente_email_tm[$num2] or...
} # fine for $num2
...
#echo "<br><br>".var_dump($header)."<br><br>";
$mittente = "";
...
#if (strstr($mittente,"<")) $mittente = substr(strstr($mittente,"<"),1,-1);
$testo = utf8_encode(imap_qprint(imap_body($email_conn,$mess[$num1],FT_PEEK)));
if (@get_magic_quotes_gpc()) $testo = stripslashes($testo);
$testo = htmlspecialchars($testo);
$testo = aggslashdb($testo);
if (stristr($testo,"Content-Type: multipart/alternative;") and stristr($testo,"Content-Type: text/plain;")) {
$testo = explode("------=_Part",$testo);
for ($num2 = 0 ; $num2 < count($testo) ; $num2++) {
if (stristr($testo[$num2],"Content-Type: text/plain;")) {
$testo = str_replace("\r","\n",str_replace("\r\n","\n",$testo[$num2]));
if (strstr($testo,"\n\n")) $testo = (strstr($testo,"\n\n"));
break;
} # fine if (stristr($testo[$num2],"Content-Type: text/plain;"))
} # fine for $num2
} # fine if (stristr($testo,"Content-Type: multipart/alternative;") and...
else {
if (stristr($testo,"Content-Type: text/html;")) {
$testo = strip_tags(str_replace("<br>","\n",str_replace("<BR>","\n",str_replace("</div>","\n",str_replace("</DIV>","\n",str_replace("</tr>","\n",str_replace("</TR>","\n",$testo)))))));
} # fine if (stristr($testo,"Content-Type: text/html;"))
...
if (!$num_persone) $num_persone = trova_numero_vicino($testo,$fr_persone);
} # fine if (!$num_persone)
} # fine if (!$num_persone)
...
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Scarica messaggi email",$pag)."\" style=\"font-size: 70%;\">
</div></form>";
} # fine if (numlin_query($server_email_tab_messaggi))
...
$orario_tab_messaggi = esegui_query("select valpersonalizza from $tablepersonalizza where idpersonalizza = 'mostra_orario_tab_messaggi' and idutente = '$id_utente'");
$orario_tab_messaggi = risul_query($orario_tab_messaggi,0,'valpersonalizza');
...
echo "<div class=\"widgetbox\" style=\"font-size:12pt;\">
                            <h4 class=\"widgettitle\"> $stringa_pagine </h4>
                            <div class=\"widgetcontent\">

<table class=\"table responsive\" >
<tr>
<thead>
<th align=\"center\">".mex("N°",$pag)."</th>
<th align=\"center\">".mex("Mittente",$pag)."</th>
<th align=\"center\">".mex("Testo",$pag)."</th>
<th align=\"center\">".mex("Data",$pag)."</th>
<th align=\"center\">".mex("Azioni",$pag)."</th></thead></tr>";
...
$tasto_prenota .= "<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Inserisci la prenotazione",$pag)."\">
</div></form><br>";
...
$tasto_prenota .= "<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Controlla disponibilità",$pag)."\">
</div></form><br>";
...
<span class=\"field\">
<select class=\"uniformselect\" name=\"numero_contratto\" >
$option_contratti
</select></span><br>

<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Visualizza il documento",$pag)."\">
</div></form><br>";
...
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Modifica",$pag)."\"><br>
-------------------------<br></div></form>";
...
<input type=\"hidden\" name=\"cambia_qualcosa\" value=\"SI\">
<input type=\"hidden\" name=\"idmessaggi\" value=\"$idmessaggi\">
<input class=\"btn btn-primary\" type=\"submit\" name=\"elimina_messaggio\" value=\"".mex("Elimina",$pag)."\">
</div></form></td></tr>";
...
<input type=\"hidden\" name=\"cambia_qualcosa\" value=\"SI\">
<input type=\"hidden\" name=\"elimina_tutti_mess\" value=\"SI\">
<input class=\"btn btn-primary\" type=\"submit\" value=\"".mex("Elimina tutti i messaggi",$pag)."\"><br>
".mex("arrivati",$pag)."<span class=\"field\"> <select class=\"uniformselect\" name=\"prima_dopo\">
<option value=\"prima\">".mex("prima del",$pag)."</option>
<option value=\"dopo\">".mex("dopo il",$pag)."</option>
</select></span>

<span class=\"field\">
<select name=\"data_arrivo\" class=\"uniformselect\">
...
<span class=\"field\">
 <select name=\"destinatario\" class=\"uniformselect\">
<option value=\"tutti\" selected>".mex("tutti",$pag)."</option>
$option_select_utenti
</select></span>
 ".mex("da mostrare dopo le",$pag)."
 
 <span class=\"field\" <select class=\"uniformselect\" name=\"ora_visione\">";
...
echo "</select></span>:<span class=\"field\"> <select class=\"uniformselect\" name=\"min_visione\">";
for ($num1 = 0 ; $num1 < 60 ; $num1 = $num1 + 15) {
if (strlen($num1) == 1) $num1 = "0".$num1;
if ($num1 <= $min_corrente and ($num1 + 15) > $min_corrente) echo "<option value=\"$num1\" selected>$num1</option>";
else echo "<option value=\"$num1\">$num1</option>";
} # fine for $num1
echo "</select></span> ".mex("il",$pag)."
<span class=\"field\">
 <select class=\"uniformselect\" name=\"data_visione\">
".str_replace("-".$data_corrente."\">","-".$data_corrente."\" selected>",$option_date)."
</select></span>-<span class=\"field\"><select class=\"uniformselect\" name=\"anno_visione\">
...
<table><tr><td style=\"height: 3px;\"></td></tr></table>".mex("testo del messaggio",$pag).":
 <input class=\"form-control input-sm\" name=\"testo\" size=\"55\" type=\"text\">&nbsp;&nbsp;&nbsp;
<input class=\"btn btn-primary\" type=\"submit\" name=\"spedisci_messaggio\" value=\"".mex("Spedisci",$pag)."\">
</div></form>";
...
</div></form><table><tr><td style=\"height: 20px;\"></td></tr></table></div></div>";
